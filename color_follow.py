...
def initTrackbars():
    ...
    # "Button" as a toggle: 0 = Stop, 1 = Start
...
last_t = time.time()
ema_x_err = None
prev_motor_on = False  # for edge-detect on the "button"
...
try:
    while True:
        ...
        if motor_on and not prev_motor_on:
            # Rising edge: just turned ON
            pid_turn.reset()
            ema_x_err = None
            last_t = time.time()
        elif not motor_on and prev_motor_on:
            # Falling edge: turned OFF -> hard stop
            stop()

        prev_motor_on = motor_on

        if center and motor_on:
            last_seen_t = time.time()
            ...
            cv2.putText(display, f"RUN x:{x_err} r:{r_err} L:{left_cmd:.2f} R:{right_cmd:.2f}",
                        (10,25), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,255,255), 2)

        else:
            # Either target lost or motor is off
            if not motor_on:
                stop()
                cv2.putText(display, "Motor: STOP (toggle to 1 to start)",
        ...
        if key == ord('q'):
            break
        # Optional quick keyboard toggle: press 'm' to toggle Motor
        if key == ord('m'):
